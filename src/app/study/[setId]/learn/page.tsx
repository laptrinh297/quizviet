...
export default function LearnPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setSrMap(map)
      buildQuestions(setData.terms, map)
    }).finally(() => setIsLoading(false))
  }, [setId])

  const buildQuestions = (allTerms: Term[], map: Map<string, SRData>) => {
    // Prioritize terms due for review or not studied yet
    ...
    const qs: Question[] = selected.map(term => {
  ...
  const handleAnswer = async (userAnswer: string) => {
    ...
    const correct = q.term.definition.trim().toLowerCase()
  ...
  const handleWritten = (e: React.FormEvent) => {
    e.preventDefault()
  ...
  const handleKnown = async () => {
    const q = questions[currentIdx]
    setKnownIds(prev => new Set([...prev, q.term.id]))
    ...
    handleAnswer(q.term.definition)
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 mb-2">Kết quả của bạn</p>
          ...
          <div className="flex flex-col gap-3">
            <Button onClick={() => buildQuestions(terms, srMap)} size="lg" className="w-full">
              <RotateCcw size={16} />
              Học tiếp
            </Button>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 mb-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-gray-400 uppercase tracking-wide">
              {q.type === 'mcq' ? 'Chọn định nghĩa đúng' : 'Nhập định nghĩa'}
            </span>
          </div>
          <p className="text-2xl font-bold text-gray-900 text-center py-4">{q.term.term}</p>
        </div>
        ...
        {q.type === 'mcq' ? (
          <div className="grid grid-cols-1 gap-3">
            {q.choices?.map(choice => {
              ...
              if (feedback) {
                if (choice === q.term.definition) {
                  btnClass += 'border-green-500 bg-green-50 text-green-800'
                } else if (choice === selectedChoice && feedback === 'wrong') {
              ...
              return (
                <button key={choice} className={btnClass} onClick={() => handleMCQ(choice)} disabled={!!feedback}>
                  {choice}
                </button>
              )
            })}
          ...
          <form onSubmit={handleWritten} className="space-y-4">
            <input
              value={answer}
            ...
            {feedback === 'wrong' && (
              <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3">
                ...
                <p className="text-sm font-semibold text-green-800">{q.term.definition}</p>
              </div>
            )}
            {!feedback && (
